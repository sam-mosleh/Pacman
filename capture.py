...
import util
from game import (Actions, Agent, Configuration, Directions, Game,
                  GameStateData, Grid, reconstituteGrid)
...
SONAR_NOISE_VALUES = [
    i - (SONAR_NOISE_RANGE - 1) / 2 for i in range(SONAR_NOISE_RANGE)
]
            ...
            if agentState.numCarrying > 0 and not agentState.isPacman:
                ...
                for index in range(state.getNumAgents()):
                    ...
                    if index in state.getRedTeamIndices():
                        ...
                        blueCount += agentState.numReturned
                if redCount >= (TOTAL_FOOD / 2) - MIN_FOOD or blueCount >= (
...
def replayGame(layout, agents, actions, display, length, redTeamName,
               blueTeamName):
    rules = CaptureRules()
...
if __name__ == '__main__':
    """
  The main function called when capture.py is run
